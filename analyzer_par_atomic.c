...
static int load_manifest(HashTable *ht, const char *manifest){
    ...
    fclose(fp); printf("[ATOM] Manifest carregado: %zu URLs\n", added); return 1;
}
...
int main(int argc, char **argv){
    ...
    printf("[ATOM] HashTable size=%zu (urls=%zu)\n", ht_size, urls);
    ...
    while(1){
        ...
        if(processed_total % 1000000L == 0){
            printf("[ATOM] Processadas %ld linhas...\n", processed_total);
        }
    ...
    printf("[ATOM] Total processado: %ld linhas. Tempo: %.3fs\n", processed_total, t1-t0);

    printf("[ATOM] Salvando resultados em %s\n", out);
